...
onready var FLOOR = $Floor.tile_set.find_tile_by_name("floor_0") 
func _ready():
	generate()
...
func connect_leaves(leaf1, leaf2):
	...
	for i in range(x, x+w):
		for j in range(y, y+h):
			if(get_cell(i, j) == WALL): set_cell(i, j, FLOOR)
...
func clear_deadends():
	...
	while !done:
		...
		for cell in get_used_cells():
			if get_cellv(cell) != FLOOR: continue
...
func check_nearby(x, y):
	var count = 0
	if get_cell(x, y-1)   == WALL:  count += 1
	if get_cell(x, y+1)   == WALL:  count += 1
	if get_cell(x-1, y)   == WALL:  count += 1
	if get_cell(x+1, y)   == WALL:  count += 1
	return count
