...
var WALL = preload('res://map_object/Wall.tscn')
var FLOOR = preload('res://map_object/Floor.tscn')
...
func fill_wall() -> void:
	for x in range(0, map_w):
		for y in range(0, map_h):
			...
			wall.position = map_to_world(Vector2(x, y))
	...
	for i in range(rooms.size()):
		...
		for x in range(r.x, r.x + r.w):
			for y in range(r.y, r.y + r.h):
				for _wall in walls.get_children():
					if map_to_world(Vector2(x, y)) == _wall.position:
						walls.remove_child(_wall)
						_wall.queue_free()
				var f = FLOOR.instance()
				f.position = map_to_world(Vector2(x, y))
				floors.add_child(f)
...
func connect_leaves(leaf1, leaf2) -> void:
	...
	for i in range(x , x + w):
		for j in range(y, y + h):
			for _wall in walls.get_children():
				if map_to_world(Vector2(i, j)) == _wall.position:
					walls.remove_child(_wall)
					_wall.queue_free()
			var f = FLOOR.instance()
			f.position = map_to_world(Vector2(i, j))
			floors.add_child(f)
